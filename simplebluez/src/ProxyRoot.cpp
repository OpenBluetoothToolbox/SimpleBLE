...
ProxyRoot::ProxyRoot(std::shared_ptr<SimpleDBus::Connection> conn, const std::string& bus_name, const std::string& path)
...
void ProxyRoot::load_managed_objects() {
...
std::vector<std::shared_ptr<Adapter>> ProxyRoot::get_adapters() {
    return std::dynamic_pointer_cast<ProxyOrg>(path_get("/org"))->get_adapters();
}

std::shared_ptr<Agent> ProxyRoot::get_agent() { return std::dynamic_pointer_cast<Agent>(path_get("/agent")); }

void ProxyRoot::register_agent() { std::dynamic_pointer_cast<ProxyOrg>(path_get("/org"))->register_agent(_agent); }

std::shared_ptr<SimpleDBus::Proxy> ProxyRoot::path_create(const std::string& path) {
    auto child = std::make_shared<ProxyOrg>(_conn, _bus_name, path);
    return std::static_pointer_cast<SimpleDBus::Proxy>(child);
}

std::shared_ptr<SimpleDBus::ObjectManager> ProxyRoot::object_manager() {
