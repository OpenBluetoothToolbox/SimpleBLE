...
void AdapterLinux::scan_start() {
    ...
    adapter_->set_on_device_updated([this](std::shared_ptr<SimpleBluez::Device> device) {
        ...
        if (this->seen_peripherals_.count(device->address()) == 0) {
            ...
            SAFE_CALLBACK_CALL(this->callback_on_scan_found_, build(peripheral));
        } else {
            SAFE_CALLBACK_CALL(this->callback_on_scan_updated_, build(peripheral));
        }
    });
...
std::vector<std::shared_ptr<PeripheralBase>> AdapterLinux::scan_get_results() {
    return collect_values(seen_peripherals_);
}

std::vector<std::shared_ptr<PeripheralBase>> AdapterLinux::get_paired_peripherals() {
    std::vector<std::shared_ptr<PeripheralBase>> peripherals;
