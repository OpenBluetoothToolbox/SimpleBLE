...
void Peripheral::write_request(BluetoothUUID const& service, BluetoothUUID const& characteristic, ByteArray data) {
    if (!is_connected()) {
        throw Exception::OperationFailed();
    }
...
void Peripheral::write_command(BluetoothUUID const& service, BluetoothUUID const& characteristic, ByteArray data) {
    if (!is_connected()) {
        throw Exception::OperationFailed();
    }
