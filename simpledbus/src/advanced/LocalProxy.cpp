...
void LocalProxy::interfaces_load(Holder managed_interfaces) {
...
void LocalProxy::interfaces_unload(SimpleDBus::Holder removed_interfaces) {
    std::scoped_lock lock(_interface_access_mutex);
...
bool LocalProxy::interfaces_loaded() {
    std::scoped_lock lock(_interface_access_mutex);
...
void LocalProxy::path_add(const std::string& path, SimpleDBus::Holder managed_interfaces) {
    ...
    if (Path::is_child(_path, path)) {
        ...
        std::shared_ptr<LocalProxy> child = path_create(path);
        ...
        if (child_result != _children.end()) {
            ...
            std::shared_ptr<LocalProxy> child = path_create(child_path);
...
bool LocalProxy::path_remove(const std::string& path, SimpleDBus::Holder options) {
...
void LocalProxy::path_append_child(const std::string& path, std::shared_ptr<LocalProxy> child) {
