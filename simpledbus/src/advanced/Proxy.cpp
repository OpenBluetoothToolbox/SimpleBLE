...
void Proxy::message_handle(Message& msg) {
    ...
    if (msg.is_signal("org.freedesktop.DBus.Properties", "PropertiesChanged")) {
        ...
        std::cout << "Received GetAll: " << msg.to_string() << std::endl;
        return;
    ...
    std::cout << "Message not handled by self: " << msg.to_string() << std::endl;
    return;
}


SimpleDBus::Holder Proxy::_collect_managed_objects(const std::string& base_path) {
